...
export default function EditModal({isOpen, onOpenChange,column_id}:{isOpen: boolean, onOpenChange: () => void,column_id : string}) {

  const [name,setName] = useState("");
  const [error,setError] = useState("");
  const [isLoading,setIsLoading] = useState(false);
  const { editColumn} = UseColumns();
  ...
  const handleAddColumn = async (e : any, onClose: () => void) => {
    ...
    try{
        setIsLoading(true);
        // Corrected line: Pass newName instead of name
        await editColumn({newName: name, projectId, columnId : column_id});
    }
    catch(error : any) {
