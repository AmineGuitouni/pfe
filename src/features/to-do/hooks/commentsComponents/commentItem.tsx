...
import { HiReply } from 'react-icons/hi'; // Added Reply Icon
import ReplyComponent from '../../components/taskComponents/taskModalComponents/replyComponent';

export default function CommentItem({ comment, isOwner, deleteFunction, editFunction, LikeDislikeComment,addFunction ,replies}: { comment: Comment, isOwner: boolean, deleteFunction: (comment_id: string) => void, editFunction: (comment_id: string, body: string) => void, LikeDislikeComment: (comment_id: string, like_dislike : string) => void,addFunction : (body: string, first_name: string, last_name: string, reply_to?: string) => Promise<void>,replies : Comment[] }) { // Corrected prop name and signature
  ...
  const [isReplying, setIsReplying] = useState(false); // State for reply form visibility
    ...
    <div className="flex items-start space-x-3 pb-4 border-b border-light_blue-500/20 last:border-b-0 pr-1">
      ...
      <div className="min-w-0 flex-1">
            ...
            <div className='w-full flex flex-col'>
              ...
              <div className=" flex items-center space-x-4 text-sm justify-end mt-2"> {/* Added mt-2 for spacing */}
                <button
                  ...
                  className="flex items-center space-x-1 hover:scale-105 transition-all ease-linear text-light_blue hover:text-light_blue-400"
                >
                  <HiReply className='cursor-pointer flex-shrink-0 h-5 w-5'/>
                  <span>Replies {replies.length > 0 ? "("+replies.length+")" : "" }</span> {/* Removed hardcoded count */}
                </button>
                <button
                  title="Like"
        ...
        <Transition
          show={isReplying}
          as={Fragment}
          enter="transition ease-out duration-200" // Slightly longer duration
          enterFrom="transform opacity-0 -translate-y-2" // Start invisible and slightly shifted up
          enterTo="transform opacity-100 translate-y-0" // End fully visible and in position
          leave="transition ease-in duration-150" // Slightly longer duration
          leaveFrom="transform opacity-100 translate-y-0" // Start fully visible and in position
          leaveTo="transform opacity-0 -translate-y-2" // End invisible and slightly shifted up
        >
          <ReplyComponent 
            replies={replies || []} 
            addReply={addFunction} 
