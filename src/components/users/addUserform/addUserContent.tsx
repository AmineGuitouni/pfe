...
import { Alert, Card, CardBody, cn } from "@heroui/react";
...
function getErrorMessage(errorType: string | null): string {
  switch (errorType) {
    ...
    case 'EMAIL_MISMATCH':
      return 'There was a mismatch with your email address. Please ensure you\'re using the correct invitation link.';
    default:
      return 'We encountered an unexpected error while Adding you. Please try again.';
  }
}

export default async function AddUserContent({ token }: { token: string | null }) {
    ...
    const states = {
      success: {
        ...
        message: "you can now enter your credentials to get started.",
        action: (
    ...
    return (
      <>
      <Card className={cn("bg-white/10 border-white/20 border-1", verificationStatus === 'success' ? '' : 'mt-20')}>
